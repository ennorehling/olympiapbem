 ...
 */
  ...
  if (game && *game) {
    if (acct->game_free != acct->orig_game_free) {
      sprintf(fnam, "%s/%s/free_%s", acct_dir, acct->player, game);
...
list_accounts(double max_balance, int cc_only, char *game, int free_flag) {
  ...
  while ((e = readdir(d)) != NULL) {
    ...
    printf("%s  %6.2lf    %6.2lf    %s\n",
           acct->player,
           acct->balance, acct->free, get_name_email(acct->player));
     ...
     char **argv;
{
  ...
  /* Make all created files readable by group. */
  (void) umask(007);
