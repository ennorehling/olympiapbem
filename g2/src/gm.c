...
gm_show_gold(int pl) {
  ...
  if (sum > 0) {
    ...
    out(pl, "Taxes:                %10s %3d%%", comma_num(gold_taxes),
        gold_taxes * 100 / sum);
    out(pl, "Times press:          %10s %3d%%", comma_num(gold_times),
        gold_times * 100 / sum);
...
gm_land_stats(int pl) {
  ...
  for (i = 0; i < ilist_len(l); i++)
    out(pl, "%10s  %s", comma_num(bx[l[i]]->temp), just_name(l[i]));
  out(pl, "%10s  %s", "======", "");
...
list_all_items(int pl) {
  ...
  out(pl, "%4s %-24s %6s %4s %4s %4s", "item", "name", "weight", "land",
      "ride", "fly");
  ...
  loop_item(i) {
    out(pl, "%4s %-24s %6s %4s %4s %4s",
        ...
        comma_num(item_ride_cap(i)), comma_num(item_fly_cap(i)));
  }
  next_item;
