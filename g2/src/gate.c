...
#ifdef NOTYET
  departure message ? arrival message ?
#endif
  ...
  if (gate) {
    ...
    if (p && kind(p->notify_jumps) == T_char)
      wout(p->notify_jumps, "%s has jumped%s through %s.",
           box_name(who), backwards ? " backwards" : "", box_name(gate));
  }
}
...
v_teleport(struct command *c) {
  ...
  if (diff_region(c->who, dest)) {
    wout(c->who, "It is not possible to teleport there from here.");
    return FALSE;
  }
