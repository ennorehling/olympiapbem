     ...
     struct item_ent **a, **b;
{
  ...
  return ((b_attack + b_missile + b_defense) -
          (a_attack + a_missile + a_defense));
}
    ...
    default:
      ...
      if ((combat_swampy || combat_sea) &&
          (f->kind == item_elite_guard ||
           f->kind == item_cavalier || f->kind == item_knight)) {
        f->attack -= 50;
        f->defense -= 50;
        assert(f->attack > 0);
        assert(f->defense > 0);
      ...
      if (combat_rain &&
          (f->kind == item_archer ||
           f->kind == item_horse_archer || f->kind == item_elite_arch)) {
        f->missile = 0;
      }
                ...
                attacker, int defender, struct fight **l_a)
{
  ...
  loop_here(where, i) {
    if (kind(i) == T_char &&
        ...
        stack_leader(i) == i &&
        attacker != i &&
        player(attacker) != player(i) && !contains_unit(*l, i)) {
 ...
 */
int
loc_target(int target)
{
  int who = 0;

  if (subkind(target) == sub_city &&
      garrison_here(province(target)) && province_admin(province(target))) {
    who = garrison_here(province(target));
  }
  else if (kind(target) == T_loc && loc_depth(target) == LOC_province) {
    if (garrison_here(province(target)) && province_admin(province(target))) {
      who = garrison_here(province(target));
    };
  }
...
contains_priest(struct fight **l)
{
  ...
  for (i = 0; i < ilist_len(l); i++)
    if (l[i]->kind == FK_noble && is_priest(l[i]->unit))
      return 1;
...
contains_mu_or_undead(struct fight **l)
{
  ...
  for (i = 0; i < ilist_len(l); i++) {
    ...
    if (l[i]->kind == FK_noble) {
      if (is_magician(l[i]->unit) && !char_hide_mage(l[i]->unit))
        return 1;
      continue;
    };
...
num_defenders(struct fight *f, struct fight **enemy, int type)
{
   ...
   *
   */
  if (type != SPECIAL && type != UNCANNY && f->behind)
    return 0;
   ...
   *
   */
  if (type == UNCANNY && (f->kind == FK_noble || !f->behind || !f->missile))
    return 0;
...
total_defenders(struct fight **l, struct fight **enemy, int type)
{
   ...
   *
   */
  while (type != SPECIAL && type != UNCANNY && sum == 0 && advance_behind(l)) {
    for (i = 0; i < ilist_len(l); i++)
      sum += num_defenders(l[i], enemy, type);
  }
 ...
 */
static int
do_magic_combat_attack(struct fight **l_a, struct fight *f,
                       struct fight **l_b, int skill)
{
  struct fight *g;
 ...
 */
static int
do_magic_combat_attacks(struct fight **l_a, struct fight *f,
                        struct fight **l_b)
{
  int sum = 0;
                 ...
                 int attack, int defense)
{
  ...
  if (get_effect(f->unit, ef_edge_of_kireus, 0, 0) &&
      ...
      (f->kind == item_swordsman ||
       ...
       f->kind == item_knight ||
       f->kind == item_elite_guard ||
       f->kind == item_cavalier || f->kind == item_pirate)) {
    attack += 2500;
  };
  ...
  if (g->inside &&
      ...
      loc_moat(l_b[0]->unit) &&
      type == MISSILE && !is_siege_engine(f->kind)) {
  ...
  if (f->kind == FK_noble) {
    ...
    case MISSILE:
      defense +=
        (100 * combat_artifact_bonus(f->unit, CA_N_MISSILE_D, &item));
      break;
    case SPECIAL:
      defense +=
        (100 * combat_artifact_bonus(f->unit, CA_N_SPECIAL_D, &item));
      break;
    default:
      break;
    }
    ...
    case MISSILE:
      defense +=
        (100 * combat_artifact_bonus(f->unit, CA_M_MISSILE_D, &item));
      break;
    case SPECIAL:
      defense +=
        (100 * combat_artifact_bonus(f->unit, CA_M_SPECIAL_D, &item));
      break;
    default:
      break;
    }
  ...
  if (n > attack) {
    out(combat_pl, "    %s.%d failed to hit %s.%d (%d/%d)",
        ...
        box_code_less(g->unit), g->kind, attack, attack + defense);
    return 0;
  }
...
do_attack(int attacker, struct fight **l_a, struct fight **l_b, int type)
{
  ...
  if (uncanny == 1) {
    ...
    wout(VECT,
         "    With uncanny accuracy, %s fires into the back row %s times!",
...
attack_round(struct fight **l_a, struct fight **l_b, int type)
{
  ...
  out(combat_pl, "  %s phase:  Attacks = %d",
      type == MELEE ? "Melee" : "Missile", total_attacks);

  out(combat_pl, "  %s phase:  Defenders = %d",
      type == MELEE ? "Melee" : "Missile", num_defend_b);
  ...
  out(combat_pl, "  %s phase: %d hits out of %d attacks.",
      type == MELEE ? "Melee" : "Missile", sum, total_attacks);
...
special_attack(struct fight **l_a, int i, struct fight **l_b, int type)
{
   ...
   */
  if (round > 1 &&
      f->kind == FK_noble && f->behind && has_magic_combat_attack(f->unit)) {
   ...
   */
  if (round > 1 &&
      f->kind == FK_noble && f->behind && has_skill(f->unit, sk_brew_fiery)) {
  ...
  if (f->kind == FK_noble &&
      ...
      has_item(f->unit, item_warmount) > 0 && !f->behind) {
    int num_defend, man;
    struct fight *g;
    print_special_banner();
  ...
  for (i = 0; i < ilist_len(l_a); i++) {
    ...
    if (g->kind == FK_fort) {
      ...
      wout(VECT, "      %s takes %s point%s damage.", box_name(g->unit),
           nice_num(g->tmp_num), g->tmp_num > 1 ? "s" : "");
    }
      ...
      if (!has_skill(g->unit, sk_personal_fttd) ||
        ...
        wout(VECT, "      %s heroically continues to fight!",
             box_name(g->unit));
      };
#endif
    }
      ...
      g->num -= g->tmp_num;
      wout(VECT, "      %s loses %s %s.", box_name(g->unit),
           nice_num(g->tmp_num), plural_item_box(g->kind, g->tmp_num));
      /*
      ...
      if (g->kind == item_skirmisher || g->kind == item_peasant ||
        ...
        if (brk) {
          wout(VECT, "        %d %s break and leave the battle!",
               brk, plural_item_box(g->kind, brk));
          g->num -= brk;
        };
      };
    };
    ...
    if (g->num <= 0 && g->protects >= 0) {
      ...
      out(combat_pl, "    %s.%d no longer protects %s",
          ...
          g->kind, box_code_less(l_a[g->protects]->unit));
  ...
  for (i = 0; i < ilist_len(l_a); i++) {
    ...
    if (item > 0) {
      ...
      if (inherit != MATES_SILENT &&
          ...
          l_b && side_has_skill(l_b, sk_capture_beasts)) {
 ...
 */
  ...
  for (i = 0; i < ilist_len(l_a); i++) {
    if (l_a[i]->kind == FK_noble &&
        subkind(l_a[i]->unit) == sub_garrison && l_a[i]->nprot < 1) {
...
run_combat(struct fight ***lap, struct fight ***lbp)
{
  ...
  if (total_attackers(l_b, l_a, MISSILE) > 0) {
    wout(VECT, "Initial missile round for the defense (%s):",
         box_name(lead_char(l_b)));
  ...
  while (num_a > thresh_a && num_b > thresh_b) {
    ...
    if (tmp != NO_ATTACKS) {
      ...
      wout(VECT, "    %s's forces hit with %s missile%s.",
           just_name(lead_char(l_a)), nice_num(tmp), tmp > 1 ? "s" : "");
    };
    tmp = attack_round(l_b, l_a, MISSILE);
    if (tmp != NO_ATTACKS) {
      if (first) {
      ...
      wout(VECT, "    %s's forces hit with %s missile%s.",
           just_name(lead_char(l_b)), nice_num(tmp), tmp > 1 ? "s" : "");
    };
    resolve_dead(l_a);
    resolve_dead(l_b);
    ...
    if (tmp != NO_ATTACKS) {
      wout(VECT, "    %s's forces hit %s time%s.", just_name(lead_char(l_b)),
           nice_num(tmp), tmp > 1 ? "s" : "");
    }
    else {
 ...
 */
int
v_prac_control(struct command *c)
{
  int ret = oly_parse(c,
 ...
 */
int
v_attack_tactics(struct command *c)
{
  int ret;
  struct skill_ent *p;
  ...
  ret = oly_parse(c, sout("practice %d", sk_attack_tactics));
  assert(ret);
...
#if 0
int
d_attack_tactics(struct command *c)
{
  struct skill_ent *p;
...
#if 0
int
v_defense_tactics(struct command *c)
{
  struct skill_ent *p;
...
v_defense_tactics(struct command *c)
{
  ...
  ret = oly_parse(c, sout("practice %d", sk_defense_tactics));
  assert(ret);
 ...
 */
int
do_trap_attack(int who, int num_attacks, int chance)
{
 ...
 */
int
v_use_beasts(struct command *c)
{
  int ret;

  ret = oly_parse(c, sout("practice %d", sk_use_beasts));
  assert(ret);
 ...
 */
int
v_prac_protect(struct command *c)
{
  int ret = oly_parse(c,
