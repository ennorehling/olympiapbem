...
is_holy_terrain(int n, int where)
{
  ...
  return (ht == subkind(province(where))) ||
    (ht == subkind(loc(where))) || (ht == subkind(where));
};
...
add_piety(int who, int amount, int use_limit)
{
   ...
   *
   */
  if (use_limit && options.piety_limit && options.head_priest_piety_limit) {
    /*
...
d_create_holy_symbol(struct command *c)
{
  ...
  sprintf(new_name, "Holy symbol of %s",
          rp_relig_skill(is_priest(c->who))->name);
...
d_bless_fort(struct command *c)
{
  ...
  if ((where != c->a) && (province(c->who) != province(c->a))) {
    wout(c->who,
         "You must be in the same province as a building to bless it.");
    return FALSE;
  };
...
d_weaken_fort(struct command *c)
{
  ...
  if ((where != c->a) && (subloc(c->who) != subloc(c->a))) {
    wout(c->who,
         "You must be in the same location as a building to weaken it.");
    return FALSE;
  };
...
v_weaken_fort(struct command *c)
{
  ...
  if ((where != c->a) && (province(c->who) != province(c->a))) {
    wout(c->who,
         "You must be in the same province as a building to weaken it.");
    return FALSE;
  };
...
d_generic_trap(struct command *c)
{
  ...
  if (!traps[i].type) {
    wout(c->who, "Priests of %s cannot set traps.",
         god_name(is_priest(c->who)));
    return FALSE;
  };
  ...
  if (is_holy_terrain(c->who, subloc(c->who))) {
    if (!add_effect
        (subloc(c->who), ef_religion_trap, is_priest(c->who), 300, 1)) {
      wout(c->who, "For some reason, the prayer fails to take effect.");
      return FALSE;
    };
    ...
    if (!add_effect
        (province(c->who), ef_religion_trap, is_priest(c->who), 300, 1)) {
      wout(c->who, "For some reason, the prayer fails to take effect.");
      return FALSE;
    };
    wout(c->who, "A %s trap is now set in %s.", traps[i].name,
...
d_improve_logging(struct command *c)
{
  ...
  if (subkind(target) != sub_forest && subkind(target) != sub_yew_grove) {
    wout(c->who,
         "You may only bless a forest or a yew grove with this prayer.");
    return FALSE;
  };

  if (province(target) != province(c->who)) {
    wout(c->who,
         "You must be in the same province as the logging operation.");
    return FALSE;
  };
...
d_improve_fort(struct command *c)
{
  ...
  if ((where != c->a) && (province(c->who) != province(c->a))) {
    wout(c->who,
         "You must be in the same province as a building to improve it.");
    return FALSE;
  };
...
d_improve_smithing(struct command *c)
{
  ...
  if (subloc(c->who) != subloc(c->a)) {
    wout(c->who,
         "You must be in the same location as the smith you are blessing.");
    return FALSE;
  };
...
d_edge_of_kireus(struct command *c)
{
  ...
  wout(c->who, "Kireus now blesses the edged weapons of %s.",
       box_name(c->who));
...
d_draw_crowds(struct command *c)
{
  ...
  if (!add_effect
      (subloc(c->who), ef_improve_production, 0, 30, item_postulant)) {
    wout(c->who, "For some reason, the prayer fails to take effect.");
    return FALSE;
  };

  if (!add_effect
      (subloc(c->who), ef_improve_production, 0, 30, item_petty_thief)) {
    wout(c->who, "For some reason, the prayer fails to take effect.");
    return FALSE;
  };
...
d_mesmerize_crowd(struct command *c)
{
  ...
  loop_here(where, i) {
    ...
    if (rnd(1, 100) < 35) {
      wout(i,
           "You find yourself captivated for 7 days by %s's mesmerizing speech!",
           box_name(c->who));
...
d_guard_loyalty(struct command *c)
{
  ...
  wout(c->who, "%s is now unswervingly loyal for one month.",
       box_name(target));
  return TRUE;
};
...
d_conceal_location(struct command *c)
{
  ...
  if ((where != c->a) && (province(c->who) != province(c->a))) {
    wout(c->who,
         "You must be in the same province as a location to conceal it.");
    return FALSE;
  };
  ...
  if (has_item(province(where), item_peasant) >= 100) {
    wout(c->who,
         "You may only conceal a sublocation in a wilderness province.");
    return FALSE;
  };
...
d_gather_holy_plant(struct command *c)
{
  ...
  if (rnd(1, 100) < 50) {
    wout(c->who, "You find a %s and collect it.",
         box_name(holy_plant(c->who)));
...
d_dedicate_temple(struct command *c)
{
  ...
  if (p == NULL) {
    wout(c->who,
         "For some reason, your god refuses to dedicate this structure.");
    return FALSE;
  }
  else {
    ...
    wout(c->who, "%s now dedicated to %s.",
         box_name(where), god_name(is_priest(c->who)));
  };
  return TRUE;
};
...
d_hinder_med_b(struct command *c)
{
  ...
  wout(c->who, "Successfully cast %s on %s.",
       box_name(sk_hinder_med), box_code(target));
...
d_vision_reg(struct command *c)
{
  ...
  if (loc_shroud(province(target)) >= rp_skill(c->use_skill)->piety) {
    wout(c->who, "%s is shrouded from your vision.", box_code(target));
