...
show_carry_capacity(int who, int num)
{
  ...
  tagout(who, "<tag type=carry_capacity unit=%d weight=%d "
         ...
         w.total_weight,
         w.land_weight,
         w.land_cap, w.ride_weight, w.ride_cap, w.fly_weight, w.fly_cap);
...
show_item_skills_sup(int who, int item, struct item_magic *p)
{
  ...
  for (i = 0; i < ilist_len(p->may_study); i++) {
    ...
    days = sout(", %d day%s instruction",
                p->orb_use_count, p->orb_use_count == 1 ? "" : "s");
#endif
  ...
  for (i = 0; i < ilist_len(p->may_use); i++) {
    ...
    tagout(who, "<tag type=may_use id=%d skill=%d parent = %d>",
           item,
           (sk != parent && has_skill(who, parent) < 1) ? 0 : sk, parent);
...
show_char_inventory(int who, int num, char *prefix)
{
  ...
  loop_inv(num, e) {
    ...
    if (first) {
      ...
      out(who, "%s%9s  %-30s %9s", prefix, "qty", "name", "weight");
    ...
    if (!prefix[0])
      tagout(who,
             "<tag type=inventory unit=%d item=%d qty=%d weight=%d extra=\"%s\">",
    ...
    out(who, "%s%9s  %-30s %s%9s  %s",
        ...
        tab_to(45), comma_num(weight), extra_item_info(who, e->item, e->qty));
  ...
  if (count > 0) {
    out(who, "%s%9s  %-30s %9s", prefix, "", "", "======");
    out(who, "%s%9s  %-30s %9s", prefix, "", "", comma_num(total_weight));
  }
    ...
    if (kind(n) == T_char && is_prisoner(n)) {
      if (first) {
        out(who, "Prisoners:      %s%s%s%s",
            box_name(n), nation_s(n), deserted_s(n), prisoner_health(n));
        first = FALSE;
      }
      else
        out(who, "                %s%s%s%s",
      ...
      tagout(who, "<tag type=prisoner id=%d prisoner=%d "
             ...
             num, n, nation_s(n), deserted_s(n), char_health(n));
...
char_rep_misc(int who, int num)
{
   ...
   *
   */
  if (has_skill(num, sk_smuggle_goods) || has_skill(num, sk_smuggle_men)) {
    s = " nothing";
...
char_rep_magic(int who, int num, char *prefix)
{
  ...
  if (ma < mea)
    out(who, "%sMaximum aura:   %d (%d+%d)", prefix, mea, ma, mea - ma);
  else
    out(who, "%sMaximum aura:   %d", prefix, ma);

  if (char_abil_shroud(num) > 0)
    out(who, "%sAbility shroud: %d aura", prefix, char_abil_shroud(num));

  if (is_loc_or_ship(char_proj_cast(num)))
    out(who, "%sProject cast:   %s", prefix, box_name(char_proj_cast(num)));

  if (char_quick_cast(num))
    out(who, "%sQuicken cast:   %d", prefix, char_quick_cast(num));
...
char_rep_religion(int who, int num)
{
  ...
  if (is_priest(num)) {
    ...
    if (ilist_len(rp_char(num)->religion.followers) > 0) {
      out(who, "Followers:  %s",
          box_name(rp_char(num)->religion.followers[0]));
      for (i = 1; i < ilist_len(rp_char(num)->religion.followers); i++)
        out(who, "            %s",
            box_name(rp_char(num)->religion.followers[i]));
    };
  }
  else if (is_follower(num)) {
    out(who, "Dedicated to:   %s (%s)",
        box_name(is_priest(is_follower(num))), box_name(is_follower(num)));
  };
  tagout(who, "<tag type=religion id=%d followers=%d priest=%d>",
         num,
         is_priest(num) ? ilist_len(rp_char(num)->religion.followers) : 0,
         is_follower(num));
}
...
show_unclaimed(int who, int num)
{
  ...
  loop_inv(num, e) {
    ...
    out(who, "%9s  %-30s %s%9s  %s",
        comma_num(e->qty),
        plural_item_box(e->item, e->qty),
        tab_to(42), comma_num(weight), extra_item_info(0, e->item, e->qty));
  }
  next_inv;
...
unit_summary_sup(int pl, int who)
{
  ...
  if (options.output_tags > 0) {
    sprintf(buf,
            ...
            fight, pr ? -1 : stack_parent(who), nam);
    tagout(pl, buf);
  };
...
loc_stack_rep_sup(int pl, int where, int who)
{
  ...
  out(pl, "%-34s %s%s%s%s", where_s, tab_to(35), ind, box_name(who), star);
  if (where)
    bx[loc(where)]->temp = 0;
}
...
loc_stack_report(int pl)
{
  ...
  if (loc_stack_explain) {
    out(pl, "");
    out(pl, "%-34s    %s%s", "",
        tab_to(38), "* -- unit belongs to another faction");
  }
...
player_report_sup(int pl)
{
  ...
  out(pl, "Study points:  %d", p->jump_start);
...
rep_player(int pl)
{
  ...
  tagout(pl, "<tag type=unit_report id=%d name=\"%s\">", pl, box_name(pl));
  tagout(pl, "<tag type=header>");
  lines(pl, s);
  tagout(pl, "</tag type=header>");
...
rep_char(int pl, ilist l)
{
  ...
  for (i = 0; i < ilist_len(l); i++) {
    if (subkind(l[i]) == sub_dead_body || subkind(l[i]) == sub_lost_soul) {
      tags_off();
      s = sout("%s~%s", p_misc(l[i])->save_name, box_code(l[i]));
      tags_on();
      t = sout("%s~%s", p_misc(l[i])->save_name, box_code(l[i]));
    }
    else {
      tags_off();
      s = box_name(l[i]);
...
player_banner()
{
  ...
  loop_player(pl) {
    ...
    if (p->email)
      out(pl, "To: %s (%s)", p->email, p->full_name ? p->full_name : "???");
    out(pl, "Subject: Olympia:TAG game %d, turn %d report", game_number,
        sysclock.turn);
    out(pl, "");
#endif
    ...
    wout(pl, "Olympia:TAG game %d turn %d report for %s.",
         game_number, sysclock.turn, box_name(pl));
...
report_account_out(int pl, int who)
{
  ...
  sprintf(cmd, "%s -a %s -A %s-old -g tag%d -p %s -s 4 > %s",
          ...
          options.accounting_dir, game_number, box_code_less(pl), fnam);
  system(cmd);
...
charge_account()
{
  ...
  loop_player(pl) {
    ...
    sprintf(cmd,
            "%s -g tag%d -a %s -A %s-old -p %s -t%s -y \"Olympia TAG turn %d\"",
            options.accounting_prog, game_number, options.accounting_dir,
            options.accounting_dir, box_code_less(pl), turn_charge(pl),
            sysclock.turn);

    if (system(cmd)) {
      out(gm_player, "Could not charge %s.", box_code(pl));
    };
  }
  next_player;
