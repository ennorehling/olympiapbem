...
set_lord(int who, int new_lord, int k, int lev)
{
  ...
  if (new_lord && new_pl != old_pl) {
    ...
    qsort(p_player(new_pl)->units,
          ilist_len(p_player(new_pl)->units), sizeof (int), int_comp);
...
unit_deserts(int who, int to_who, int loy_check, int k, int lev)
{
  ...
  if (sp && nation(sp) &&
      ...
      !rp_nation(nation(to_who))->neutral && nation(sp) != nation(to_who))
    must_pay = FALSE;
  ...
  if (to_who && !is_real_npc(to_who) && nation(to_who)) {
    ...
    loop_char_skill_known(who, e) {
      if (ilist_lookup(rp_nation(nation(to_who))->proscribed_skills, e->skill)
          != -1
   ...
   */
  if (to_who &&
      is_prisoner(who) && player(to_who) == player(stack_parent(who))) {
...
v_bribe(struct command *c)
{
  ...
  if (player(target) == player(c->who)) {
    wout(c->who, "%s already belongs to our faction.", box_name(target));
    return FALSE;
  }
...
thanks_for_gift(int who, int target)
{
  ...
  switch (rnd(1, 3)) {
  case 1:
    wout(who, "%s graciously accepts our gift.", box_name(target));
    break;

  case 2:
    wout(who, "%s thanks us for the gift.", box_name(target));
    break;
  ...
  case HEAD_FOR_HILLS:
    ...
    if (player(target) != deserted_player &&
      ...
      wout(c->who, "%s left the service of %s, but didn't "
           "join us.", box_name(target), box_name(player(target)));
    };
    break;
...
v_oath(struct command *c)
{
  ...
  if (flag == 1 && lk == LOY_oath && lr == 1) {
    wout(c->who, "%s already is at %s.", box_name(c->who), loyal_s(c->who));
    return FALSE;
  }
...
v_incite(struct command *c)
{
  ...
  if (stack_parent(mob)) {
    wout(c->who, "%s is stacked under a leader.", box_name(mob));
    return FALSE;
  }
  ...
  if (rnd(1, 3) == 1) {
    ...
    loop_here(where, i) {
      ...
      wout(i, "Rumors claim that %s is trying to incite "
           "a mob to attack %s.", box_name(c->who), box_name(target));
    }
    next_here;
  }
