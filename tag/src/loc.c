...
province(int who) {
  ...
  while (who > 0 && (kind(who) != T_loc || loc_depth(who) != LOC_province))
    who = loc(who);
...
viewloc(int who) {
  ...
  while (who > 0 &&
         ...
         subkind(who) != sub_graveyard && subkind(who) != sub_faery_hill) {
    who = loc(who);
  }
...
remove_from_here_list(int loc, int who) {
   ...
   *
   */
  if (!ilist_len(rp_loc_info(loc)->here_list) && rp_subloc(loc)) {
