...
v_meditate(struct command *c)
{
  ...
  if (char_cur_aura(c->who) >= max_eff_aura(c->who)) {
    wout(c->who, "Current aura is already %d.  It may not "
         "be increased further via meditation.", char_cur_aura(c->who));
    return FALSE;
  }
...
v_write_spell(struct command *c)
{
  ...
  if (unteachable) {
    wout(c->who,
         "%s is an unteachable subskill, so you cannot write it down.",
         box_name(spell));
    return FALSE;
  };
  ...
  if (has_item(c->who, item_blank_scroll) < scrolls_needed(days)) {
    wout(c->who, "You need %d scroll%s to record %d day%s.",
         scrolls_needed(days),
         scrolls_needed(days) > 1 ? "s" : "", days, days > 1 ? "s" : "");
    return FALSE;
  };
  ...
  if (book && valid_box(book) && has_item(c->who, book) &&
      (p = rp_item_magic(book)) && ilist_lookup(p->may_study, spell)) {
  ...
  if ((magic_skill(c->use_skill) || religion_skill(c->use_skill)) &&
    ...
    wout(c->who,
         "Some odd warp in the space-time continuum aborts this command.");
    return FALSE;
  };
  ...
  wout(c->who, "Spend %s writing %s into %s.",
       weeks(c->wait), box_name(spell), box_name(c->c));
