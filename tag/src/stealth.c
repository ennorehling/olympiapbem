...
d_seek(struct command *c) {
  ...
  if (target) {                 /* target specified */
     ...
     *
     */
    if (has_skill(target, sk_hide_self) && char_really_hidden(target)) {
      chance -= 5 * skill_exp(target, sk_hide_self);
    };

    if ((subloc(c->who) == subloc(target)) && rnd(1, 100) < chance) {
      add_contact(target, c->who);
 ...
 */
  loop_here(subloc(c->who), i) {
    ...
    tmp = chance;
    if (has_skill(i, sk_hide_self) && char_really_hidden(i)) {
...
v_conceal_nation(struct command *c) {
  ...
  if (!i) {
    wout(c->who, "The prefix \"%s\" doesn't match any nation.", c->parse[1]);
    return FALSE;
  };
  ...
  if (get_effect(c->who, ef_conceal_nation, 0, 0)) {
    ...
    wout(c->who,
         "You remove your previous disguise to reveal your true nation.");
  };
...
d_assassinate(struct command *c) {
  ...
  if (loc_depth(subloc(c->who)) == LOC_province &&
      weather_here(subloc(c->who), sub_fog) && !contacted(target, c->who)) {
  ...
  loop_stack(stack_leader(target), i) {
    if (i != target && skill_exp(i, sk_protect_noble)) {
  ...
  if (rnd(1, 100) < chance) {
    ...
    wout(c->who, "Your assassination attempt fails!");
