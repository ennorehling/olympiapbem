...
effect_list_print(FILE * fp, char *header, struct effect **l)
{
  ...
  for (i = 0; i < ilist_len(l); i++) {
    ...
    fprintf(fp, "%d %d %d %d",
            l[i]->type, l[i]->subtype, l[i]->days, l[i]->data);
  }
...
effect_list_scan(char *s, struct effect ***l)
{
  ...
  while (1) {
    new = my_malloc(sizeof (*new));
    sscanf(s, "%d %d %d %d",
           &new->type, &new->subtype, &new->days, &new->data);
...
build_list_print(FILE * fp, char *header, struct entity_build **l)
{
  ...
  for (i = 0; i < ilist_len(l); i++) {
    ...
    fprintf(fp, "%d %d %d %d",
            l[i]->type,
            l[i]->build_materials, l[i]->effort_required, l[i]->effort_given);
  }
...
build_list_scan(char *s, struct entity_build ***l)
{
  ...
  while (1) {
    ...
    sscanf(s, "%d %d %d %d",
           &new->type,
           &new->build_materials, &new->effort_required, &new->effort_given);
 ...
 */
      ...
      fprintf(fp, "%d %d %d %d %d %d %d %d %d",
              ...
              l[i]->cloak,
              l[i]->have_left,
              l[i]->month_prod, l[i]->old_qty, l[i]->counter);
    }
...
trade_list_scan(char *s, struct trade ***l, int box_num)
{
  ...
  while (1) {
    ...
    sscanf(s, "%d %d %d %d %d %d %d %d %d",
           ...
           &new->cost,
           &new->cloak,
           &new->have_left, &new->month_prod, &new->old_qty, &new->counter);
...
static void
print_religion(FILE * fp, struct entity_religion_skill *p)
{
  fprintf(fp, " RL\n");
  fprintf(fp, " na %s\n", p->name);
  fprintf(fp, " st %d\n", p->strength);
...
static struct entity_religion_skill *
scan_religion(int box_num)
{
  char *t;
  int c;
...
write_leftovers()
{
  ...
  if (fp != NULL) {
    for (i = 0; i < MAX_BOXES; i++)
      if (bx[i] != NULL && kind(i) != T_nation && bx[i]->temp == 0) {
        if (kind(i) != T_deleted)
          save_box(fp, i);
      }
...
load_system()
{
   ...
   *
   */
  if (!options.top_piety && !options.middle_piety && !options.bottom_piety) {
