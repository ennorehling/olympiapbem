  ...
  for (num = rnd(1, 3); num > 0; num--) {
    ...
    loop_skill(new) {
      if (skill_school(new) == new &&
          !magic_skill(new) &&
          !religion_skill(new) &&
          learn_time(new) < 14 && rnd(1, ++count) == 1) {
        skill = new;
      };
    }
    next_skill;
...
seed_city_trade(int where)
{
  ...
  if (prov_kind == sub_plain) {
    ...
    add_city_trade(where, CONSUME, item_leather, rnd(3, 6), rnd(125, 135), 0);

  if (prov_kind == sub_mountain) {
    add_city_trade(where, PRODUCE, item_iron, rnd(1, 2), rnd(75, 200), 0);
  }
...
nearby_city_two(ilist cities, int a, int b)
{
  ...
  for (i = 0; i < ilist_len(cities); i++) {
    if (cities[i] == a || cities[i] == b || reg != region(cities[i]))
      continue;
...
long_route(ilist cities, int item)
{
  ...
  printf("%s from %s-%d <%s> to %s-%d <%s>, d=%d g=%d\n",
         ...
         ilist_len(consume), box_code_less(two), dist_r, dist_g);
...
seed_long_routes()
{
  ...
  for (item = 0; rare_trade_items[item] && reg + 1 < reg_len; item++) {
    ...
    for (i = 0; i < ilist_len(all_cities); i++)
      if (region(all_cities[i]) == one || region(all_cities[i]) == two)
...
seed_common_tradegoods()
{
  ...
  loop_subkind(sub_region, reg) {
    ...
    for (i = 0, j = 0; i < ilist_len(cities) &&
      ...
      qty = (base_price(item) * item_weight(item) % 40) + rnd(6, 12);
...
seed_rare_tradegoods()
{
  ...
  loop_subkind(sub_region, reg) {
    ...
    for (i = 0, j = 0; i < ilist_len(cities) && j < ilist_len(goods); i++) {
      if (rnd(0, 1))
        continue;
      ...
      if (rnd(0, 1) == 0) {
        fprintf(stderr, "%s sold in %s for %s\n",
                ...
                box_name(cities[i]), comma_num(base_price(item)));

        add_city_trade(cities[i], PRODUCE, item, qty,
                       base_price(item) - rnd(0, 1), rnd(1, NUM_MONTHS));
      }
      else {
        if (rnd(0, 1))
          qty /= 2;
        ...
        fprintf(stderr, "%s bought in %s for %s\n",
                box_name_qty(item, qty),
                box_name(cities[i]), comma_num(base_price(item) + premium));
      }
    }
  }
  next_subkind;
...
seed_population()
{
  ...
  loop_loc(where) {
    ...
    if (subkind(where) != sub_forest && subkind(where) != sub_plain)
      continue;
    /*
...
seed_orcs()
{
  ...
  loop_province(where) {
    ...
    if (!found && rnd(1, 100) < 15) {
      ...
      wout(gm_player,
           "Created %d size new orc stack in %s.", total, box_name(where));
    };
  }
  next_province;
};
