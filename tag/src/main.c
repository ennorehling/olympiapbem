  ...
  loop_nation(i) {
    ...
    loop_player(pl) {
      if (rp_player(pl) &&
          ...
          nation(pl) == i &&
          rp_player(pl)->email && !rp_player(pl)->nationlist)
        fprintf(fp, "%s\n", rp_player(pl)->email);
    }
    next_player;
     ...
     *
     */
    if (rp_player(gm_player) && !rp_player(gm_player)->nationlist)
...
write_player_list()
{
  ...
  fprintf(fp, "<BODY>\n");
  fprintf(fp,
          "<TABLE ALIGN=ABSCENTER CELLSPACING=0 CELLPADDING=5 WIDTH=\"100%%\" BGCOLOR=\"#48D1CC\" >\n");
  fprintf(fp,
          "<TR><TD><B><FONT SIZE=+1>Num</FONT></B></TD>"
          "<TD><B><FONT SIZE=+1>Faction</FONT></B></TD>"
          "<TD><B><FONT SIZE=+1>Nation</FONT></B></TD>"
          "<TD><B><FONT SIZE=+1>Email Address</FONT></B></TD>" "</TR>\n");
...
preprocess(char *in, char *out, char *args)
{
  ...
  sprintf(buf, "%s %s < %s > %s", options.cpp, args, in, out);
  err = system(buf);
  if (err) {
  ...
  loop_player(pl) {
    if (!is_real_npc(pl)) {
      ...
      fprintf(fp, "<TR><TD><A HREF=\"http://www.pbm.com/oly/tag/reports"
              ...
              game_number,
              box_code_less(pl),
              game_number, sysclock.turn, box_code_less(pl), just_name(pl));
    };
  }
  next_player;
     ...
     int pl;
{
  ...
  sprintf(fnam, "%s/%d/%s", options.html_path, game_number,
          box_code_less(pl));
  mkdir(fnam, 0777);
  umask(S_IWGRP | S_IXGRP | S_IWOTH | S_IXOTH);
...
output_html_rep(int pl)
{
  ...
  report = sout("%s/%d/%s/tag%d-%d.html",
                options.html_path,
                game_number, box_code_less(pl), game_number, sysclock.turn);
...
output_html_map(int pl)
{
  ...
  loop_known(p_player(pl)->known, i) {
    if (kind(i) == T_loc &&
        ...
        region(i) != faery_region && region(i) != cloud_region) {
