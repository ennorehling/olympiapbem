...
take_unit_items(int from, int inherit, int how_many)
{
  ...
  loop_inv(from, e) {
     ...
     *
     */
    if (rp_item(e->item) && rp_item(e->item)->ungiveable) {
     ...
     *
     */
    if (man_item(e->item) && subkind(to) == sub_ni)
      qty = 0;
  ...
  loop_inv(who, e) {
    if ((ignore_ninjas && e->item == item_ninja) ||
        (ignore_angels &&
         ...
         e->item == item_flesh_golem || e->item == item_iron_golem))
      continue;
...
item_weight(int who)
{
   ...
   *
   */
  if (is_artifact(who) && rp_item_artifact(who)->type == ART_WEIGHTLESS) {
...
add_item_weight(int item, int qty, struct weights *w, int mountains)
{
   ...
   *
   */
  if (rc && (item == item_new_wagon || item == item_war_wagon) && mountains)
    rc = 0;
...
ship_cap(int ship)
{
  ...
  if (s) {
    sc = (s->hulls * HULL_CAPACITY) -
      (s->forts * FORT_WEIGHT) -
      (s->sails * SAIL_WEIGHT) - (s->keels * KEEL_WEIGHT);
    if (sc < 0)
      sc = 0;
  }
