...
first_prisoner_pos(int where) {
  ...
  for (i = 0; i < ilist_len(p->here_list); i++)
    if (kind(p->here_list[i]) == T_char && is_prisoner(p->here_list[i])) {
      return i;
    }
...
drop_stack(int who, int to_drop) {
  ...
  if (is_prisoner(to_drop)) {
    ...
    wout(VECT, "%s dropped %s from the stack.",
         box_name(who), box_name(to_drop), just_name(to_drop));
  }
  ...
  if (release_swear_flag) {
    ...
    if (rnd(1, 5) < 5) {
      ...
      switch (rnd(1, 3)) {
      case 1:
        wout(who, "%s spits on you, and vanishes in a cloud of orange smoke.",
             box_name(to_drop));
        break;
      case 2:
        ...
        break;
      case 3:
        wout(who, "%s smiles briefly at you, then vanishes.",
             box_name(to_drop));
        break;
      }
...
take_prisoner(int who, int target) {
  ...
  if (ni)
    ...
    wout(VECT, "%s is %s by %s.",
         box_name(target), "taken prisoner", box_name(who));
...
contains_mu_undead(int i) {
  struct item_ent *j;
  if (is_magician(i) && !char_hide_mage(i))
    return 1;
  /*
...
stack_contains_mu_undead(int who) {
  ...
  loop_stack(who, i) {
    if (contains_mu_undead(i) && !is_prisoner(i))
      return 1;
  }
  next_stack;
  return 0;
...
v_stack(struct command *c) {
  ...
  if (stacked_beneath(c->who, target)) {
    wout(c->who, "Cannot stack beneath %s since %s is "
         "stacked under you.", box_name(target), just_name(target));
    return FALSE;
  }
  ...
  if (options.mp_antipathy &&
      ((stack_contains_priest(c->who) &&
       ...
       (stack_contains_priest(target) && stack_contains_mu_undead(c->who)))) {
    wout(c->who,
         "Priests cannot stack with magicians or undead, and vice versa.");
    return FALSE;
  };
