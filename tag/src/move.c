...
departure_message(int who, struct exit_view *v)
{
  ...
  if (!is_loc_or_ship(v->destination) || !is_loc_or_ship(v->orig))
    return;
...
move_exit_land(struct command *c, struct exit_view *v, int show)
{
  ...
  if (w.ride_cap >= w.ride_weight && !swamp) {
    ...
    if (w.land_weight > w.land_cap * 2) {
      ...
      wout(c->who,
           "You have a total of %d weight, and your maximum capacity is %d.",
           w.land_weight, w.land_cap * 2);
      return -1;
    }
    /*
  ...
  if (get_effect(v->destination, ef_slow_move, 0, 0)) {
    ...
    if (is_holy_terrain(c->who, v->destination)) {
      if (show) {
        wout(c->who,
             "You sense the power of %s obscuring the entrances to %s.",
...
pay_fine(struct command *c, int fine, int ruler)
{
  ...
  if (total < fine) {
    ...
    wout(VECT,
         "You pay the fine and %s is detained for questioning for 3 days.",
         box_name(c->who));
    ...
    if (is_smuggling(c->who, ef_smuggle_goods)) {
      if (check_smuggling(c->who, ef_smuggle_goods, sk_smuggle_goods)) {
        ...
        if (reduce) {
          indent += 3;
          wout(c->who,
               "You smuggle %s worth of goods unseen through the gates.",
               gold_s(reduce));
          indent -= 3;
        };
        w_cost -= reduce;
  ...
  if (control->men) {
    ...
    if (is_smuggling(c->who, ef_smuggle_men)) {
      if (check_smuggling(c->who, ef_smuggle_men, sk_smuggle_men)) {
        ...
        if (reduce) {
          indent += 3;
          wout(c->who,
               "You smuggle %s worth of men unseen through the gates.",
               gold_s(reduce));
          indent -= 3;
        };
        m_cost -= reduce;
    ...
    if (m_cost)
      wout(c->who, "Entrance fee is %s on %d men/beasts.",
           gold_s(m_cost), count_stack_any(c->who));
  }
  ...
  if (control->nobles) {
    ...
    if (is_smuggling(c->who, ef_smuggle_men)) {
      if (check_smuggling(c->who, ef_smuggle_men, sk_smuggle_men)) {
        ...
        if (reduce) {
          indent += 3;
          wout(c->who,
               "You smuggle %s worth of nobles unseen through the gates.",
               gold_s(reduce));
          indent -= 3;
        };
        n_cost -= reduce;
    ...
    if (n_cost)
      wout(c->who, "Entrance fee is %s on %d nobles.",
           gold_s(n_cost), count_stack_units(c->who));
  }
...
can_join_guild(int who, int g)
{
   ...
   *
   */
  if (is_priest(who) && g != rp_relig_skill(is_priest(who))->strength) {
  ...
  for (i = 0; i < ilist_len(p->offered); i++)
    if (!has_skill(who, p->offered[i])) {
      wout(who,
           "You must master all available skills before entering a guild.");
      return FALSE;
    };
...
check_guilds(struct command *c, struct exit_view *v)
{
  ...
  if (is_temple(v->destination) &&
    ...
    wout(c->who, "Only a priest of %s may enter %s.",
         box_name(is_temple(v->destination)), box_name(v->destination));
    return FALSE;
  };
...
peaceful_enter(int who, int from, int where)
{
   ...
   *
   */
  if (ruler && is_hostile(ruler, who) && !can_sneak(who, where)) {
    return FALSE;
  };
  ...
  if (pl &&
      ...
      !will_admit(pl, who, ruler) && !can_sneak(who, where)) {
    return FALSE;
  };
  return TRUE;
};
...
check_peaceful_move(struct command *c, struct exit_view *v)
{
  ...
  if (pl &&
      ...
      !will_admit(pl, c->who, ruler) && !can_sneak(c->who, v->destination)) {
    wout(c->who, "%s refused you admission to %s.",
         box_name(ruler), box_name(v->destination));
    return FALSE;
  }
  ...
  else if (rp_loc(v->destination) && (rp_loc(v->destination)->control.men || rp_loc(v->destination)->control.weight || rp_loc(v->destination)->control.nobles) && pl && /* This also checks for a controlled garrison */
           !will_admit(pl, c->who, v->destination) &&
           !may_rule_here(c->who, v->destination) &&
           kind(v->orig) == T_loc &&
           loc_depth(v->orig) == LOC_province &&
           province_admin(v->destination) !=
           province_admin(v->orig) && !can_sneak(c->who, v->destination)) {
    control = &rp_loc(v->destination)->control;
  };

  if (control && ruler && (control->men ||
                           control->nobles || control->weight)) {
    if (!will_pay(c->who)) {
...
v_move(struct command *c)
{
  ...
  if (!move_permitted(c, v) || !check_peaceful_move(c, v)) {
    return FALSE;
  };
...
move_stack(int who, int where)
{
  ...
  if (loc_depth(where) == LOC_province &&
      ...
      !in_hades(where) && !in_faery(where) && !in_clouds(where))
...
move_exit_water(struct command *c, struct exit_view *v, int ship, int show)
{
  ...
  case sub_roundship:
    ...
    if (n < 8) {
      ...
      if (weather_here(where, sub_wind) && delay > 1) {
        wind_bonus = 1;
        if (show)
...
#define MAX(a,b) ((a) > (b) ? (a) : (b))
  case sub_ship:
    ...
    if (sail_time == 1000 && row_time == 1000) {
      if (show)
        wout(c->who,
             "The ship is unable to be rowed or sailed at this time.");
      return -1;
    }
      ...
      if (show)
        wout(c->who,
             "The rowers dip their oars to the cadence of the drums.");
      return row_time;
    };
    break;
...
select_target_local(struct command *c)
{
  ...
  if (kind(target) == T_char) {
    ...
    if (loc_depth(where) == LOC_province &&
        weather_here(where, sub_fog) && !contacted(target, c->who)) {
...
v_move_attack(struct command *c)
{
  ...
  if (numargs(c) < 1) {
    wout(c->who, "Specify direction or destination to %s.", c->parse[0]);
    return FALSE;
  }
