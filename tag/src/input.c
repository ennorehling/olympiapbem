...
parse_line(char **l, char *s)
{
  ...
  while (*s) {
    while (*s && isspace(*s))   /* eat whitespace */
      s++;
 ...
 */
  ...
  if (char_gone(c->who) &&
      stack_leader(c->who) != c->who && c->cmd != cmd_wait)
    return TRUE;
...
do_urchin_spies(struct command *c)
{
  ...
  loop_effects(c->who, e) {
    if (e->type == ef_urchin_spy &&
        e->data == subloc(c->who) &&
        valid_box(e->subtype) && e->data == subloc(e->subtype)) {
      wout(e->subtype, "An urchin reports that %s does \"%s\".",
           box_name(c->who), c->line);
    };
  }
  next_effect;
};
    ...
    if (options.output_tags < 1)
      ...
      if (rest == NULL || *rest == '\0')
        out(c->who, "> <tag type=command name=%s>%s</tag type=command>",
            cmd_tbl[c->cmd].name, c->parse[0]);
      else
        out(c->who, "> <tag type=command name=%s>%s</tag type=command>%s",
            cmd_tbl[c->cmd].name, c->parse[0], rest);
    };

    if (c->fuzzy)
      out(c->who, "(assuming you meant '%s')", cmd_tbl[c->cmd].name);
  }
  ...
  for (pri = 0; pri < MAX_PRI; pri++) {
    for (i = 0; i < ilist_len(load_q[pri]); i++) {
      ...
      if (!is_prisoner(c->who) &&
          !char_moving(c->who) && c->second_wait == FALSE) {
        return pri;
      }
    }
  }
 ...
 */
    ...
    for (j = 0; j < ilist_len(l); j++) {
      ...
      if (c->state == LOAD &&
          c->pri == pri &&
          !is_prisoner(i) && !char_moving(i) && c->second_wait == FALSE) {
        do_command(c);
        check_all_waits();
