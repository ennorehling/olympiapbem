...
check_here() {
  ...
  loop_boxes(i) {
     ...
     *
     */
    if (where > 0 && subkind(i) != sub_region && !in_here_list(where, i)) {
  ...
  loop_skill(sk) {
    ...
    for (i = 0; i < ilist_len(p->guild); i++) {
      if (bx[p->guild[i]]->temp) {
        fprintf(stderr, "\twarning: both %s and %s "
                "offer skill %d\n",
                box_name(sk), box_name(bx[p->guild[i]]->temp), p->guild[i]);
      }
      else
      ...
      if (skill_school(p->guild[i]) != sk) {
        fprintf(stderr, "\twarning: %s offers %d, "
                ...
                p->guild[i], skill_school(p->guild[i]));
      }
    }
...
check_city() {
  ...
  loop_city(city) {
    loop_trade(city, t) {
      if (t->kind == SELL && item_unique(t->item) && !has_item(city, t->item))
...
check_db() {
  ...
  loop_city(i) {
    ...
    loop_inv(i, e) {
      if (item_unique(e->item) &&
          !find_trade(i, SELL, e->item) && !find_trade(i, BUY, e->item)) {
