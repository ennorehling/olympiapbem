...
show_chars_below_highlight(int who, int n, int depth, int where)
{
  ...
  loop_char_here(n, i) {
    tagout(who, "<tag type=char_here id=%d where=%d under=%d>", i, where, n);
...
show_chars_here(int who, int where)
{
  ...
  if (loc_depth(where) == LOC_province &&
      weather_here(where, sub_fog) && is_priest(who) != sk_domingo) {
    out(who, "");
  ...
  if (loc_depth(where) == LOC_province &&
      weather_here(where, sub_mist) && is_priest(who) != sk_domingo) {
    out(who, "");
  ...
  loop_here(where, i) {
    if (kind(i) == T_char) {
      ...
      tagout(who, "<tag type=char_here id=%d where=%d under=0>", i, where);
...
show_sublocs_here(int who, int where)
{
  ...
  loop_here(where, i) {
    if (kind(i) == T_loc) {
      ...
      tagout(who, "<tag type=subloc id=%d where=%d>", i, where);
  ...
  loop_here(where, i) {
    if (kind(i) == T_ship) {
      ...
      tagout(who, "<tag type=ship_here id=%d where=%d "
             "owner=%d>", i, where, first_character(i));
...
show_loc_skills(int who, int where)
{
  ...
  loop_loc_teach(where, i) {
    tagout(who, "<tag type=taught_skill id=%d name=\"%s\">", i, box_name(i));
...
show_weather(who, where)
{
  ...
  if (can_see_weather_here(who, where)) {
    ...
    loop_here(where, i) {
      ...
      tagout(who, "<tag type=storm id=%d loc=%d>", i, where);
      wout(who, "%s", liner_desc(i));
...
show_loc_ruler(int who, int where)
{
  ...
  if (fee) {
    tagout(who, "<tag type=fees id=%d nobles=%d weight=%d men=%d>",
           ...
           rp_loc(where)->control.weight, rp_loc(where)->control.men);
...
show_loc(int who, int where)
{
  ...
  if (subkind(where) == sub_city || is_guild(where) == sk_trading)
