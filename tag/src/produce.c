...
item_gen_here(int where, int item)
{
  ...
  for (i = 0; terr_prod[i].terr; i++)
    if (terr_prod[i].terr == subkind(where) && terr_prod[i].item == item)
      return TRUE;
  ...
  loop_here(where, enclosed) {
    encl_terr = subkind(enclosed);
    for (i = 0; terr_prod2[i].terr; i++)
      if (terr_prod2[i].terr == encl_terr && terr_prod2[i].item == item) {
        return TRUE;
      };
  }
  next_here;
...
harv_tbl[] = {
  ...
  item_corpse,
      ...
      "Corpses are found in graveyards.", "raise corpses", FALSE, 0}, {
  item_mallorn_wood,
      sk_harvest_mallorn,
      0,
      20,
  ...
  item_opium,
      ...
      "Opium is harvested only in poppy fields.", "harvest opium", TRUE, 0}, {
  item_stone,
      sk_quarry_stone,
      item_worker,
      0,
  ...
  item_fish,
      ...
      "Fish must be caught in ocean provinces.", "catch fish", TRUE, 0}, {
  item_lumber,
      sk_harvest_lumber,
      item_worker,
      0,
      ...
      "Wood must be cut in forest provinces.", "cut timber", TRUE, 0}, {
  item_yew,
      sk_harvest_yew,
      item_worker,
      0,
      ...
      "Yew must be cut in yew groves", "cut yew", TRUE, 0}, {
  item_wild_horse,
      sk_catch_horse,
      0,
      25,
  ...
  item_farrenstone,
      ...
      "Farrenstone is found in caves.", "collect farrenstone", TRUE, 0}, {
  item_pretus_bones,
      sk_collect_elem,
      0,
      25,
  ...
  item_ratspider,
      ...
      "Ratspiders are found in ruins.", "collect ratspiders", TRUE, 0}, {
...
item_avail(int where, int item)
{
  ...
  if (item == item_peasant) {
    ...
    if (num < (has_city(where) ? 1000 : 100))
      return 0;
...
v_generic_harvest(struct command *c, int number, int days, struct harvest *t)
{
  ...
  if (where == 13240 || where == 13241)
    fprintf(stderr, "\n%d: begin fish for %s\n",
            sysclock.day, box_code_less(c->who));
#endif
...
i_generic_harvest(struct command *c, struct harvest *t)
{
  ...
  if (c->d == 0 && item_avail(where, t->item) == 0) {
    ...
    out(c->who, "%s %s.", cap(t->got_em), just_name_qty(t->item, c->d));
...
v_collect(struct command *c)
{
  ...
  if (is_priest(c->who) && (item == item_peasant || item == item_postulant)) {
    ...
    if (!temple_found) {
      wout(c->who,
           "Priest may only recruit in a province with a temple of the priest's religion.");
      return FALSE;
    };
  };
...
v_grow_pop(struct command *c)
{
  ...
  if (where != subloc(c->who)) {
    wout(c->who,
         "You must be in the province to encourage the population to grow.");
    return FALSE;
  };
