...
times_masthead() {
  ...
  sprintf(turn_s, "Turn %d  %d Players", sysclock.turn, nplayers);
  {
    ...
    sprintf(issue_s, "Game %d, Season \"%s\", month %d, in the year %d.",
            game_number, month_names[month], month + 1, year + 1);
  }
  ...
  fprintf(fp,
          "   |                    Send orders to: orders@tag%d.pbm.com               |\n",
          game_number);
  fprintf(fp,
          "   +----------------------------------------------------------------------+\n\n");
  fprintf(fp,
...
times_goal_info() {
  ...
  if (options.mp_antipathy) {
    ...
    if (!priests && !mus && !others) {
      fprintf(fp,
              "    * No pieces of the Staff of the Sun have been found.\n");
    }
    else {
      if (others) {
        ...
        loop_subkind(sub_special_staff, i) {
          if (kind(item_unique(i)) == T_char &&
              !is_magician(item_unique(i)) && !is_priest(item_unique(i))) {
      ...
      if (priests) {
        fprintf(fp, "  * %s piece%s of the Staff of the Sun %s "
                "held by priests.\n", nice_num(priests),
                priests == 1 ? "" : "s", priests == 1 ? "is" : "are");
        loop_subkind(sub_special_staff, i) {
          if (kind(item_unique(i)) == T_char && is_priest(item_unique(i))) {
...
times_credit(struct command *c) {
  ...
  if (!options.times_pay)
    return TRUE;

  pl = player(c->who);

  if (times_paid(pl)) {
    wout(c->who, "The Times has already paid faction %s this month.",
         box_name(pl));
    return FALSE;
  }
