...
market_here(int who) {
  ...
  while (who > 0 && subkind(who) != sub_city &&
         !(subkind(who) == sub_guild &&
           rp_subloc(who) && rp_subloc(who)->guild == sk_trading)) {
    if (is_ship(who))
      return 0;
...
#endif
{
  ...
  if (rp_subloc(where) && garrison && !may_rule_here(seller->who, where)) {
      ...
      if (cost - treasury > 0) {
        assert(consume_item(province(buyer->who),
                            ...
                            (has_item(province(buyer->who), item_peasant) >
                             qty) ? qty : has_item(province(buyer->who),
                                                   item_peasant)));
      };
    }
    else
  ...
  if (kind(seller->who) != T_loc) {
    if (tariff) {
      wout(seller->who,
           "Sold %s%s for %s each, total of %s (%s minus %s tariff)",
           box_name_qty(item, qty), buyer_s, gold_s(item_cost),
           gold_s(cost - tariff), gold_s(cost), gold_s(tariff));
    }
    else {
      wout(seller->who, "Sold %s%s for %s each, total of %s.",
           box_name_qty(item, qty),
           buyer_s, gold_s(item_cost), gold_s(cost - tariff));
    };
  };
  ...
  if (garrison && tariff) {
    ...
    wout(seller->who, "Paid a market tax of %s to %s.",
         gold_s(tariff), box_name(where));
    wout(garrison, "Collected %s in tariffs.", gold_s(tariff));
    gold_tariffs += tariff;
  };
  ...
  for (i = 0; i < ilist_len(l); i++) {
    ...
    if (first) {
      ...
      out(who, "%5s %*s %7s %6s %9s   %-25s",
          "trade", CHAR_FIELD, "who", "price", "qty", "wt/ea", "item");
      out(who, "%5s %*s %7s %6s %9s   %-25s",
          "-----", CHAR_FIELD, "---", "-----", "---", "-----", "----");
    ...
    tagout(who, "<tag type=trade type=%d who=%d cost=%d qty=%d "
           "item=%d>",
           l[i]->kind,
           l[i]->cloak ? 0 : l[i]->who, l[i]->cost, qty, l[i]->item);
    out(who, "%5s %*s %7s %6s %9s   %-25s",
    ...
    loop_trade(where, t) {
      if (t->kind == PRODUCE && t->month_prod) {
        ...
        tagout(who, "<tag type=produces loc=%d "
               "item=%d month=%d>", where, t->item, t->month_prod);
...
list_pending_trades(int who, int num) {
  ...
  loop_trade(num, t) {
    ...
    tagout(who, "<tag type=trade id=%d kind=%d "
           "cost=%d num=%d item=%d>",
           who, t->kind, (t->cost), (t->qty), (t->item));
