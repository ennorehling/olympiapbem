...
determine_map_edges()
{
  ...
  loop_province(i) {
    if (region(i) == faery_region ||
        region(i) == hades_region || region(i) == cloud_region)
      continue;
...
exit_distance(int loc1, int loc2)
{
  ...
  if (subkind(loc1) == sub_mine_shaft
      || subkind(loc1) == sub_mine_shaft_notdone
      || subkind(loc2) == sub_mine_shaft
      || subkind(loc2) == sub_mine_shaft_notdone)
    return 0;
 ...
 */
  ...
  if (loc_depth(where) == LOC_province &&
      subkind(dest) == sub_ocean && province_has_port_city(where)) {
    v->impassable = TRUE;
  }
 ...
 */
      ...
      if (subkind(where) == sub_ocean || subkind(dest) == sub_ocean)
        v->water = TRUE;
...
exits_from_loc_nsew_select(int who, int where, int land, int rand)
{
  ...
  for (i = 0; i < ilist_len(l); i++) {
    if (((land & LAND) && !l[i]->water) || ((land & WATER) && l[i]->water))
...
list_exit_extras(int who, struct exit_view *v)
{
  ...
  if (rp_loc(v->destination) && garrison_here(v->destination)) {
    indent += 3;
  ...
  if (rp_loc(v->destination) &&
      rp_loc(v->destination)->control.men && province_admin(v->destination)) {
    indent += 6;
...
list_exits_sup(int who, int where, struct exit_view *v, char *first)
{
  ...
  tagout(who, "<tag type=exit id=%d dir=%d dest=%d inside=%d "
         ...
         v->direction,
         v->destination,
         v->inside, v->dest_hidden, v->impassable, v->distance);
...
list_road_sup(int who, int where, struct exit_view *v, char *first)
{
  ...
  out(who, "%s, to %s, %s%s",
      just_name(v->road), box_name(v->destination), hid, dist);
...
list_sailable_routes(int who, int ship)
{
  ...
  for (i = 0; i < ilist_len(l); i++)
    if (l[i]->direction > 0 &&
        (l[i]->direction != DIR_IN || see_all(who) == 2) && l[i]->water) {
...
find_hidden_exit(int who, struct exit_view **l, int which)
{
  ...
  if (l[which]->road) {
    ...
    wout(who, "A hidden inner location has been found in %s!",
         box_name(where));
    out(who, "");
