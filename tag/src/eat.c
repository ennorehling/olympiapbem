  ...
  if (options.check_balance) {
    ...
    if (result != -1 && result != 0) {
      ...
      err(EAT_ERR,
          "**                                                     **");
      err(EAT_ERR,
          "** Your account has a low balance and you cannot       **");
      err(EAT_ERR,
          "** afford to pay for your next turn.                   **");
      err(EAT_ERR,
          "**                                                     **");
      err(EAT_ERR,
          "** See http://www.pbm.com/oly/tag/payment.html         **");
      err(EAT_ERR,
          "**                                                     **");
      err(EAT_ERR,
          "*********************************************************");
...
is_safe(int who, int n)
{
   ...
   *
   */
  if (kind(n) == T_item && !item_unique(n))
    return 1;
  /*
   *  Skill
   *
   ...
   *
   */
  if (kind(n) == T_char && player(who) && is_unit(player(who), n))
    return 1;
  /*
...
buy_comment(struct command *c)
{
  if (numargs(c) < 1) {
    ...
    return sout("clear %s for %s",
                c->parse[0], safe_name(c->who, c->a, c->parse[1]));
  }
  else if (numargs(c) == 2) {
  ...
  if (numargs(c) < 2) {
    ...
    if (c->b == 0)
      ...
      return sout("drop %s %s",
                  nice_num(c->b), safe_name(c->who, c->a, c->parse[1]));
  }
  else {
    if (c->b == 0)
      return sout("drop all %s (keeping %s)",
                  safe_name(c->who, c->a, c->parse[1]), nice_num(c->c));
    else
...
catch_comment(struct command *c)
{
  ...
  ret = oly_parse(cp, sout("collect %d %d %d", item_wild_horse, c->a, c->b));
  assert(ret);
  ...
  if (numargs(c) < 2) {
    ...
    if (c->c == 0) {
      return sout("give all %s to %s, keeping %s",
                  safe_name(c->who, c->b, c->parse[2]),
                  safe_name(c->who, c->a, c->parse[1]), nice_num(c->d));
    }
    else {
...
check_arg(struct command *c, int i, int t)
{
  ...
  case CMD_unit:
     ...
     *
     */
    if (find_nation(c->parse[i]))
      return;
    if (fuzzy_strcmp(c->parse[i], "monster"))
      return;
    if (fuzzy_strcmp(c->parse[i], "monsters"))
      return;
    if (fuzzy_strcmp(c->parse[i], "garrison"))
      return;
    if (fuzzy_strcmp(c->parse[i], "garison"))
      return;
    /*
     ...
     */
    if (valid_box(args[i]) &&
        (kind(args[i]) == T_player || kind(args[i]) == T_char))
      return;
    /*
     *  Possibly an "unformed" unit.
     *
  ...
  case CMD_item:
    if (!valid_box(args[i]) || kind(args[i]) != T_item) {
      err(EAT_WARN,
          sout
          ("The %s argument of this command should be an item. %s does not appear to be an item.",
           ordinal(i), safe_name(c->who, args[i], c->parse[i])));
    };
    return;
  case CMD_skill:
    if (!valid_box(args[i]) || kind(args[i]) != T_skill) {
      err(EAT_WARN,
          sout
          ("The %s argument of this command should be a skill. %s does not appear to be an skill.",
           ordinal(i), safe_name(c->who, args[i], c->parse[i])));
    };
    return;
  case CMD_days:
  ...
  case CMD_use:
    if (!valid_box(args[i]) ||
      ...
      err(EAT_WARN,
          sout
          ("The %s argument of this command should be a skill or an item. %s does not appear to be either.",
           ordinal(i), safe_name(c->who, args[i], c->parse[i])));
    }
  ...
  case CMD_practice:
    if (!valid_box(args[i]) || kind(args[i]) != T_skill) {
      ...
      err(EAT_WARN,
          sout
          ("Note: %s doesn't currently have the %s gold required to practice %s.",
           box_name(c->who), nice_num(practice_cost(args[i])),
           safe_name(c->who, args[i], c->parse[i])));
    };
    return;
  default:
...
check_cmd(struct command *c)
{
   ...
   *
   */
  if (cmd_tbl[c->cmd].max_args && numargs(c) > cmd_tbl[c->cmd].max_args) {
...
eat(char *fnam, int mail_now)
{
  ...
  if (cpp) {
    ...
    sprintf(buf, "egrep -v '#include' %s > /tmp/cpp1.%d;"
            ...
            " > /tmp/cpp.%d",
            fnam, getpid(), options.cpp, libdir, getpid(), getpid());
    system(buf);
  ...
  if (reply_addr) {
    ...
    if (mail_now) {
      ret = system(sout("rep %s/log/%d | sendmail -t", libdir, eat_pl));
      if (ret) {
...
v_format(struct command *c)
{
  ...
  for (i = 1; i < ilist_len(c->parse) && c->parse[i]; i++) {
    ...
    if (strcasecmp("HTML", c->parse[i]) == 0) {
      ...
      p_player(plyr)->format |= FORMAT_TEXT;
...
do_rules_url(struct command *c)
{
  ...
  if (c->parse[2]) {
    ...
    out_alt_who = EAT_OKAY;
    wout(c->who ? c->who : eat_pl, "Rules HTML path cleared.");
  };
  return TRUE;
};
