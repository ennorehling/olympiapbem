...
alert_palantir_scry(int who, int where)
{
  ...
  loop_char_here(where, n) {
    ...
    wout(n, "%s used a palantir to scry this location.", box_name(who));
  ...
  loop_char_here(where, n) {
    ...
    if (has_detect)
      wout(n, "%s scried %s from %s.",
           box_name(who), box_name(where), char_rep_location(who));
  }
  next_char_here;
}
...
check_shield_artifact(int where)
{
  ...
  loop_all_here(where, n) {
    if (has_artifact(n, ART_SHIELD_PROV, 0, 0, 0))
      return TRUE;
  }
  next_all_here;

  if (has_artifact(where, ART_SHIELD_PROV, 0, 0, 0))
    return TRUE;
...
d_scry_region(struct command *c)
{
  ...
  if (!is_loc_or_ship(targ_loc)) {
    wout(c->who, "%s is no longer a valid location.", box_code(targ_loc));
    return FALSE;
  }
  ...
  if (aura <= loc_shroud(province(targ_loc))) {
    wout(c->who, "%s is shrouded from your scry.", box_code(targ_loc));
    ...
    loop_char_here(where, n) {
      ...
      if (has_skill(n, sk_shroud_region)) {
        wout(n, "%s cast %s here.  %s is now cloaked "
             ...
             box_code(where), nice_num(p->shroud));
      }
    }
    next_char_here;
  }
...
d_dispel_region(struct command *c)
{
  ...
  if (p && p->shroud > 0) {
    ...
    wout(c->who, "Removed an aura %s magical shroud from %s.",
         nice_num(p->shroud), box_name(targ_loc));
    p->shroud = 0;
    notify_loc_shroud(targ_loc);
  }
  else {
    wout(c->who, "%s was not magically shrouded.", box_name(targ_loc));
  }
...
d_locate_char(struct command *c)
{
  ...
  if (subkind(target) == sub_dead_body) {
    ...
    wout(c->who, "%s is in %s.", box_name(target), char_rep_location(target));
  }
...
v_bar_loc(struct command *c)
{
  ...
  if (loc_barrier(where) > 0) {
    wout(c->who, "%s already has a barrier.", box_name(where));
    return FALSE;
  }

  if (no_barrier(where) > 0) {
    wout(c->who, "You cannot cast another barrier on %s.", box_name(where));
    return FALSE;
  }
...
v_unbar_loc(struct command *c)
{
  ...
  if (*(c->parse[1]) == '0')
    ...
    v = parse_exit_dir(c, cast_where(c->who), sout("use %d", sk_unbar_loc));
 ...
 */
  ...
  wout(c->who, "Attempt to project next cast to %s.", box_name(to_where));
...
d_proj_cast(struct command *c)
{
  ...
  if (!is_loc_or_ship(to_where)) {
    wout(c->who, "%s is not a valid location.", box_code(to_where));
    return FALSE;
  }
...
v_use_proj_cast(struct command *c)
{
  ...
  if (im == NULL || !is_loc_or_ship(im->project_cast) || !is_magician(c->who)) {
