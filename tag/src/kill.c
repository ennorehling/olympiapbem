...
nearby_grave(int where) {
  ...
  log_write(LOG_CODE, "%s has no nearby grave", box_name(where));
...
convert_to_nothing(int cur_pl, int who) {
  ...
  if (valid_box(pl) && dead_body_np) {
    if (options.death_nps == 1) {
      add_np(pl, 1);
      out(pl, "%s~%s has passed on.  Gained 1 NP.", name, box_code(who));
    }
...
convert_to_soul(int pl, int who) {
  ...
  if (p_item(who)->plural_name)
    my_free(p_item(who)->plural_name);
...
kill_char(int who, int inherit, int status) {
  ...
  if (kind(who) != T_char || subkind(who))
    status = S_nothing;
  ...
  if (subkind(who) == sub_garrison) {
    ...
    wout(item_unique(who), "*** %s has %s ***", just_name(who),
         verbs[status]);
  }
  else if (p_char(who)) {
    int sp;
  ...
  if (!char_melt_me(who) && has_skill(who, sk_transcend_death)) {
    ...
    wout(who, "%s appears at %s.", box_name(who), box_name(hades_pit));
    return;
  };
  ...
  if (!char_melt_me(who) && (a = has_artifact(who, ART_RESTORE, 0, 0, 1))) {
    ...
    wout(who, "%s appears at %s.", box_name(who), box_name(where));
    /*
