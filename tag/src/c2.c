...
drop_player(int pl)
{
  ...
  if (save_flag && acct_flag) {
    sprintf(cmd, "%s -a %s -A %s-old -p %s -g tag%d -d",
            ...
            options.accounting_dir, box_code_less(pl), game_number);
    system(cmd);
  }
...
v_fee(struct command *c)
{
  ...
  if (!valid_box(where) || (!rp_loc(where) && !rp_subloc(where))) {
  ...
  if (kind(where) == T_loc && loc_depth(where) == LOC_province) {
     ...
     *
     */
    if (!garrison_here(where) || province_admin(province(where)) != c->who) {
...
v_board(struct command *c)
{
  ...
  if (!valid_box(owner) ||
      !(ship_fee = calc_entrance_fee(rp_subloc(ship)->control, c, owner))) {
...
do_market_tax(struct command *c)
{
  ...
  wout(c->who,
       "%s market tax rate will be set to %d%% at the end of the month.",
       box_name(city), rate);
  return TRUE;
...
do_province_tax(struct command *c)
{
  ...
  wout(c->who, "%s tax rate set to %d%%.", box_name(province), rate);
...
v_tax(struct command *c)
{
  ...
  if (subkind(where) != sub_city &&
      ...
      subkind(where) != sub_desert && subkind(where) != sub_island) {
