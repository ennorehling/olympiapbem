...
new_province_garrison(int where, int castle) {
  ...
  new = new_char(sub_garrison, 0, where, -1, garr_pl, LOY_npc, 0, "Garrison");
...
#if 0
  {
    out(c->who, "A garrison here must be bound to %s.", box_name(castle));
    return FALSE;
  }
#endif
...
garrison_summary(int pl) {
  ...
  out(pl, "%4s %5s %6s %4s %4s %4s %4s %6s %s",
      "garr", "where", "border", "men", "cost", "tax", "rate", "castle",
      "rulers");
  out(pl, "%4s %5s %6s %4s %4s %4s %4s %6s %s", "----", "-----", "------",
      "---", "----", "---", "----", "------", "------");
  tagout(pl, "</tag type=header>");
  ...
  for (i = 0; i < ilist_len(l); i++) {
    ...
    tagout(pl, "<tag type=garrison unit=%d loc=%d men=%d "
           ...
           province_admin(garr), p_loc(subloc(garr))->control.closed);
    ...
    out(pl, "%4s %5s %6s %4d %4d %4d %4d %5s  %s",
        ...
        unit_maint_cost(garr, 0),
        rp_misc(garr)->garr_tax,
        taxr, box_code_less(garrison_castle(garr)), garr_own_s(garr));
      ...
      loop_inv(garr, e) {
        if (first) {
          ...
          out(pl, "%s in %s<tag type=tab col=48>weight",
              box_name(garr), box_name(province(garr)));
          out(pl,
              "-----------------------------------<tag type=tab col=48>------");
          first = FALSE;
        };
