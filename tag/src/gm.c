...
gm_show_skill_use_counts(int pl) {
  ...
  for (i = 0; i < ilist_len(l); i++) {
    ...
    out(pl, "%4d%1c  %4s  %5s  %s",
        ...
        box_code_less(l[i]), just_name(l[i]));
  }
...
gm_show_skills_known(int pl) {
  ...
  for (i = 0; i < ilist_len(l); i++) {
    ...
    out(pl, "%4d   %5s  %4s  %s",
        bx[l[i]]->temp,
        box_code_less(l[i]),
        p->use_count ? comma_num(p->use_count) : "", just_name(l[i]));
  }
...
gm_list_animate_items(int pl) {
  ...
  loop_item(i) {
    if (!is_fighter(i) &&
        !man_item(i) && !beast_capturable(i) && !item_animal(i))
      continue;
...
gm_show_gold(int pl) {
  ...
  if (sum) {
    out(pl, "Common magic:         %10s %3d%%", comma_num(gold_common_magic),
        gold_common_magic * 100 / sum);
    out(pl, "Lead to gold:         %10s %3d%%", comma_num(gold_lead_to_gold),
        gold_lead_to_gold * 100 / sum);
    out(pl, "Pots and baskets:     %10s %3d%%", comma_num(gold_pot_basket),
        gold_pot_basket * 100 / sum);
    out(pl, "Opium:                %10s %3d%%", comma_num(gold_opium),
        gold_opium * 100 / sum);
    out(pl, "Fish:                 %10s %3d%%", comma_num(gold_fish),
        gold_fish * 100 / sum);
    out(pl, "Trade to cities:      %10s %3d%%", comma_num(gold_trade),
        gold_trade * 100 / sum);
    out(pl, "Inn income:           %10s %3d%%", comma_num(gold_inn),
        gold_inn * 100 / sum);
    out(pl, "Taxes:                %10s %3d%%", comma_num(gold_taxes),
        gold_taxes * 100 / sum);
    out(pl, "Tariffs:              %10s %3d%%", comma_num(gold_tariffs),
        gold_tariffs * 100 / sum);
    out(pl, "Fees:                 %10s %3d%%", comma_num(gold_fees),
        gold_fees * 100 / sum);
    out(pl, "Combat with indeps:   %10s %3d%%", comma_num(gold_combat_indep),
        gold_combat_indep * 100 / sum);
    out(pl, "Combat with players:  %10s %3d%%", comma_num(gold_combat),
        gold_combat * 100 / sum);
    out(pl, "Petty thievery:       %10s %3d%%", comma_num(gold_petty_thief),
        gold_petty_thief * 100 / sum);
    out(pl, "Temple income:        %10s %3d%%", comma_num(gold_temple),
        gold_temple * 100 / sum);
    out(pl, "Pillaging:            %10s %3d%%", comma_num(gold_pillage),
        gold_pillage * 100 / sum);
    out(pl, "Ferry boarding:       %10s %3d%%", comma_num(gold_ferry),
        gold_ferry * 100 / sum);
    out(pl, "Claim:                %10s %3d%%", comma_num(gold_claim),
        gold_claim * 100 / sum);
    out(pl, "Times:                %10s %3d%%", comma_num(gold_times),
        gold_times * 100 / sum);
    out(pl, "                      %10s %4s", "", "----");
  };
...
gm_show_locs_visited(int pl) {
  ...
  out(pl, "    %d%% visible, %d%% hidden",
      vis * 100 / n_sub_visit, hid * 100 / n_sub_visit);
...
gm_loyalty_stats(int pl) {
  ...
  out(pl, "%d chars: %d oath (%d%%), %d fear (%d%%), %d contract "
      ...
      fear, fear * 100 / tot, cont, cont * 100 / tot, unsw, unsw * 100 / tot);
  ...
  loop_player(i) {
    ...
    loop_units(i, j) {
      ...
      if (loc_depth(where) == LOC_build &&
          ...
          !is_ship(where) && !is_ship_notdone(where))
        sum_bld++;
    }
    next_unit;
    ...
    out(pl, "%4s %3s %5s %5s %4s %6s %5s %6s  %s",
        ...
        knum(sum_ship, TRUE), knum(sum_skills, TRUE), just_name(i));
  }
  next_player;
...
list_all_items(int pl) {
  ...
  out(pl, "%4s %-24s %3s %6s %4s %4s %4s %4s %3s", "item", "name", "mnt",
      "weight", "land", "ride", "fly", "cost", "cap");
  ...
  loop_item(i) {
    ...
    out(pl, "%4s %-24s %3d %4d %4d %4d %4d %4d %3s",
        ...
        item_ride_cap(i),
        item_fly_cap(i),
        rp_item(i)->base_price, item_animal(i) ? "yes" : "no");
  }
  next_item;
