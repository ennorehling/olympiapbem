...

  {
    ...
    for (i = 0; i < ilist_len(l) && i < NCITIES; i++) {
      ...
      add_city_trade(new, PRODUCE,
                     item_elvish_arrow, 10, 20 + rnd(0, 12) * 5, 0);
    }
...
do_wild_hunt(int where) {
  ...
  loop_here(where, i) {
    ...
    if (has_use_key(i, use_faery_stone))
      continue;
    /* Keep this one */
    if (has_artifact(i, ART_PROT_FAERY, 0, 0, 1))
      continue;
   ...
   */
  wout(who,
       "After a dizzying ride past many marvels, the Wild Hunt dumps you");
  if (i == 1000) {
    wout(who, "back in %s!", box_name(where));
    wout(where,
         "The Wild Hunt reappears and dumps %s unceremoniously to the ground.",
         box_name(who));
    return;
  };
...
v_use_faery_stone(struct command *c) {
  ...
  if (rp_item_magic(c->a)->orb_use_count < 1) {
    wout(c->who,
         "The stone gives off a blinding glow and suddenly vanishes!");
  ...
  if (subkind(subloc(c->who)) == sub_faery_hill) {
    ...
    if (ilist_len(l) == 0) {
       ...
       */
      wout(c->who,
           "The stone glows brightly and the way to Faery is opened.");
...
v_use_faery_artifact(struct command *c) {
  ...
  if (subkind(subloc(c->who)) == sub_faery_hill) {
    ...
    if (ilist_len(l) == 0) {
      ...
      wout(c->who, "%s glows briefly, but nothing happens.", box_name(c->a));
    };
    return TRUE;
  }
  else {
...
update_faery() {
  ...
  for (i = 0; i < ilist_len(p->npc_memory); i++) {
     ...
     *
     */
    if (!valid_box(p->npc_memory[i]) || kind(p->npc_memory[i]) != T_char) {
