  ...
  loop_storm(i) {
    ...
    if (options.output_tags)
      out(pl, "<tag type=storm storm=%d kind=%s owner=%d "
          "loc=%d strength=%d>",
          i, subkind_s[subkind(i)], owner, where, storm_strength(i));
...
dissipate_storm(int storm, int show) {
   ...
   *
   */
  if (owner && kind(owner) == T_char && region(owner) == region(storm))
...
d_dissipate(struct command *c) {
  ...
  out(c->who, "Current piety is now %s.",
      nice_num(rp_char(c->who)->religion.piety));
...
d_lightning(struct command *c) {
  ...
  if (is_loc_or_ship(target)) {
    ...
    if (is_priest(target) == sk_eres) {
      wout(VECT, "Lightning strikes %s but does no harm!", box_name(target));
    }
    else {
      wout(VECT, "%s was struck by lightning!", box_name(target));
...
d_death_fog(struct command *c) {
  ...
  loop_inv(target, e) {
    ...
    if (item_attack(e->item) > 0 &&
      ...
      wout(target, "%s %s %s.", cap(nice_num(amount)),
           plural_item_name(e->item, amount), fog_excuse());
      aura_used += amount;
    };
  } next_inv;
  ...
  wout(c->who, "Killed %s men of %s.",
       nice_num(aura_used), aura_used == 1 ? "man" : "men", box_name(target));
