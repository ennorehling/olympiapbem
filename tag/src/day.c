...
give_stack_losses(int who, int evac_levels) {
  ...
  wout(VECT, "You must evacuate through %s level%s of collapsing mine shaft!",
       nice_num(evac_levels), evac_levels == 1 ? "" : "s");
  loop_stack(who, i) {
    loop_inv(i, e) {
      /*
      ...
      if (rp_item(e->item)->is_man_item || rp_item(e->item)->animal) {
        wout(i, "%s %s %s killed in the evacuation.",
             cap(nice_num(losses)),
             plural_item_name(e->item, losses), losses == 1 ? "was" : "were");
      }
      else {
  ...
  loop_loc(where) {
    ...
    case sub_mine_shaft:
      ...
      if (sysclock.day == mine_collapse_day &&
          valid_box(where) &&
          rnd(1, 30) == 1 && !get_effect(where, ef_protect_mine, 0, 0))
...
lose_monsters(int mtype, char *msg, int max_loss, int chance) {
  ...
  loop_char(who) {
    ...
    wout(who, msg, cap(nice_num(loss)), plural_item_name(mtype, loss),
         (loss > 1) ? "" : "s");
...
reseed_monster_provinces() {
  ...
  if (monsters < provinces / 2) {
    ...
    log_write(LOG_MISC, "No need to reseed monsters in provinces.");
  };
};
  ...
  loop_skill(religion) {
    ...
    loop_char(who) {
      ...
      if (staff_bonus) {
        wout(who, "Your god rewards you with %s piety for the parts "
             "of the Staff of the Sun you hold.",
             nice_num(pow(2, staff_bonus)));
    ...
    for (i = 3; i < (3 + (ilist_len(priests) - 2) / 2); i++) {
      wout(priests[i], "As a faithful priest, %s blesses you with %s piety.",
           god_name(religion), nice_num(options.bottom_piety));
    ...
    for (i = (3 + (ilist_len(priests) - 2) / 2); i < ilist_len(priests); i++) {
      wout(priests[i],
           "%s is disappointed that you haven't brought more faithful to the church.",
           god_name(religion));
...
do_pen_crown() {
  ...
  if (!pen_crown) {
    ...
    if (item_unique(pen_crown) != pen_noble) {
      wout(item_unique(pen_crown),
           "The Gods of the Pen pass the crown to a new king.");
      wout(pen_noble, "The Gods of the Pen select you as the new king.");
    };
...
loyalty_decay() {
  ...
  loop_char(who) {
    ...
    case LOY_summon:
      p->loy_rate--;
      if (p->loy_rate <= 0 && !get_effect(who, ef_guard_loyalty, 0, 0)) {
        leave_stack(who);
    ...
    case LOY_contract:
      ...
      if (p->loy_rate <= 0 &&
         ...
         */
        int pl = player(who);
        if (has_item(pl, item_gold) >= amount - has_item(who, item_gold)) {
      ...
      if (p->loy_rate <= 0 &&
          !get_effect(who, ef_guard_loyalty, 0, 0) && charge(who, amount)) {
        wout(who, "%s honors himself with %s gold.",
             box_name(who), nice_num(amount));
        p->loy_rate += amount;
      };
      if (p->loy_rate <= 0 &&
          rnd(1, 2) == 1 && !get_effect(who, ef_guard_loyalty, 0, 0)) {
        log_write(LOG_DEATH, "%s deserts, %s", box_name(who), loyal_s(who));
        (void) unit_deserts(who, deserted_player, TRUE, LOY_unsworn, 0);
        continue;
      }
...
temple_income() {
  ...
  loop_temple(i) {
    ...
    if (owner == 0)
      continue;
    if (!is_priest(owner) ||
        !is_temple(i) || is_priest(owner) != is_temple(i)) {
    ...
    gold_temple += income;
    wout(owner, "%s collected offerings of %s.", box_name(i), gold_s(income));
  }
  next_temple;
}
...
collect_taxes() {
  ...
  loop_loc(where) {
    ...
    if (garr &&
       ...
       */
      amount =
        (rp_loc(where)->tax_rate * has_item(where, item_peasant)) / 100.0;
      if (amount < 1)
        amount = 1;
      /*
...
add_bounty(int where) {
  ...
  wout(gm_player, "Added a bounty for %s at %s.",
       plural_item_name(choice, 2), box_name(where));
};
...
update_bounties() {
  ...
  loop_city(where) {
    ...
    count = 0;
    loop_trade(where, t) {
      if (t->kind == CONSUME && subkind(t->item) == sub_animal_part) {
        count++;
        /*
...
open_faery_hill(int where) {
  ...
  loop_province(i) {
    ...
    if (subkind(i) < sub_forest || subkind(i) > sub_swamp)
      continue;
    if (province_subloc(i, sub_faery_hill))
      continue;
    if (province_subloc(i, sub_city))
      continue;
    sum++;
    if (rnd(1, sum) == 1) {
      dest = i;
...
update_loc_fees() {
  ...
  loop_loc(i) {
     ...
     *
     */
    if (rp_subloc(i) && rp_subloc(i)->tax_market != rp_subloc(i)->tax_market2) {
    ...
    if (region(where) == hades_region) {
      loop_char_here(where, i) {
        ...
        if (subkind(i) == sub_lost_soul)
          continue;
        if (noble_item(i) && subkind(noble_item(i)) == sub_undead)
          continue;
        if (noble_item(i) && subkind(noble_item(i)) == sub_demon_lord)
          continue;
        if (noble_item(i) && subkind(noble_item(i)) == sub_lost_soul)
          continue;
        if (has_skill(i, sk_transcend_death))
          continue;
        loss = rnd(1, 10);
        if (is_priest(i)) {
          if (!use_piety(i, loss)) {
          ...
          wout(i,
               "The unshielded magical energies overcome your magical powers!");
          kill_char(i, MATES, S_soul);
        }
        else {
