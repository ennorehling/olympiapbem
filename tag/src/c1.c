...
v_name(struct command *c)
{
  ...
  wout(c->who, "%s will now be known as %s.", old_name, box_name(target));

  if (target != c->who && (kind(target) == T_char || is_loc_or_ship(target)))
    wout(target, "%s will now be known as %s.", old_name, box_name(target));
...
how_many(int who, int from_who, int item, int qty, int have_left)
{
  ...
  if (num_has <= 0) {
    wout(who, "%s has no %s.", just_name(from_who), just_name(item));
  ...
  if (num_has <= have_left) {
    wout(who, "%s has only %s.",
         just_name(from_who), just_name_qty(item, num_has));
    return 0;
  }
...
v_accept(struct command *c)
{
  ...
  for (i = 0; i < ilist_len(p->accept); i++) {
    if (p->accept[i]->item == item &&
        p->accept[i]->from_who == from_who && p->accept[i]->qty == qty) {
...
will_accept_sup(int who, int item, int from, int qty)
{
  ...
  if (p) {
    for (i = 0; i < ilist_len(p->accept); i++) {
      ...
      int qty_match = (p->accept[i]->qty >= qty || p->accept[i]->qty == 0);
...
flag_raised(int who, char *flag)
{
  ...
  for (i = 0; i < ilist_len(flags); i++) {
    if (who != 0 && player(flags[i]->who) != who && flags[i]->who != who)
      continue;
...
check_wait_conditions(struct command *c)
{
  ...
  for (i = 0; i < ilist_len(c->wait_parse); i++) {
    ...
    case 1:                    /* day n */
      ...
      if (cond) {
        if (not)
          return sout("today is%s day %d.", not ? " not" : "", p->a1);
        else
    ...
    case 22:                   /* turn n */
      ...
      if (cond) {
        if (not)
          return sout("it is%s turn %d.", not ? " not" : "", p->a1);
        else
    ...
    case 2:                    /* unit n */
      ...
      cond = not - cond;
      if (cond)
        return sout("%s is%s here.", box_code(p->a1), not ? " not" : "");
      break;
      ...
      if (cond) {
        if (not)
          return sout("%s doesn't have %s.",
                      just_name(c->who), gold_s(p->a1));

        return sout("%s has %s.",
                    just_name(c->who), gold_s(has_item(c->who, item_gold)));
      }
      break;
    ...
    case 4:                    /* item n q */
      ...
      if (cond) {
        if (not)
          return sout("%s doesn't have %s.",
                      just_name(c->who), just_name_qty(p->a1, p->a2));

        return sout("%s has %s.",
                    just_name(c->who),
                    just_name_qty(p->a1, has_item(c->who, p->a1)));
      }
      break;
      ...
      if (not) {
        ...
        if (j >= 0)
          return sout("%s signaled '%s'",
                      box_name(flags[j]->who), flags[j]->flag);
      }
      break;
    }
    ...
    case 6:                    /* loc */
      ...
      cond = not - cond;
      if (cond)
        return sout("%sat %s.", not ? "not " : "", box_name(p->a1));
      break;
    ...
    case 20:                   /* shiploc */
    {
      ...
      if (!is_loc_or_ship(p->a1))
        return sout("%s is not a location or ship.", box_code(p->a1));
    ...
    case 8:                    /* top */
      ...
      cond = not - cond;
      if (cond)
        return sout("we are%s the stack leader", not ? " not" : "");
      break;
    ...
    case 9:                    /* ferry n */
      ...
      cond = not - cond;
      if (cond)
        return sout("the ferry has%s signaled.", not ? " not" : "");
    ...
    case 11:                   /* rain */
      ...
      cond = not - cond;
      if (cond)
        return sout("it is%s raining.", not ? " not" : "");
      break;
    ...
    case 12:                   /* fog */
      ...
      cond = not - cond;
      if (cond)
        return sout("it is%s foggy.", not ? " not" : "");
      break;
    ...
    case 13:                   /* wind */
      ...
      cond = not - cond;
      if (cond)
        return sout("it is%s windy.", not ? " not" : "");
      break;
    ...
    case 23:                   /* mist */
      ...
      cond = not - cond;
      if (cond)
        return sout("it is%s misty.", not ? " not" : "");
      break;
    case 14:                   /* not */
      not = TRUE;
      setnot = TRUE;
    ...
    case 15:                   /* owner */
      ...
      cond = not - cond;
      if (cond)
        return sout("we are%s the first character here", not ? " not" : "");
      break;
    ...
    case 19:                   /* clear */
      ...
      cond = not - cond;
      if (cond)
        return sout("it is%s clear.", not ? " not" : "");
      break;
    ...
    case 25:                   /* alone */
      ...
      cond = not - cond;
      if (cond)
        return sout("you are%s alone.", not ? " not" : "");
      break;
